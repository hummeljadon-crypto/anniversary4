...
class SoundEngine {
  ...
  heartbeat(strength = 1) {
    this.tone(58, 42, .16, "sine", .12 * strength);
    setTimeout(() => this.tone(72, 48, .12, "sine", .09 * strength), 170);
  }

  letterSeal() {
    this.tone(420, 260, .28, "triangle", .07);
    setTimeout(() => this.tone(620, 420, .34, "sine", .055), 120);
  }
}
...
async function triggerChaseJumpscare() {
  ...
  audio.noise(.42,.2,760);
  audio.tone(210,42,.52,"sawtooth",.12);
  flash();
  shake();
  buzz([110,35,150]);
...
function startBreath(event) {
  ...
  breathTimer = setInterval(async () => {
    ...
    const beatWindow = Math.floor(elapsed / (progress > 78 ? 700 : progress > 52 ? 950 : 1250));
    if (beatWindow !== startBreath.lastBeatWindow) {
      startBreath.lastBeatWindow = beatWindow;
      audio.heartbeat(progress > 78 ? 1.35 : progress > 52 ? 1.1 : .85);
    }
...
async function prepareFinisher() {
  ...
  audio.noise(.8,.055,520);
  audio.tone(130,44,1.2,"sawtooth",.085);
  flash();
  ...
  audio.tone(260,420,.36,"triangle",.045);
}
...
$("#finishBtn").onclick = async () => {
  ...
  audio.tone(92,52,1.1,"sawtooth",.065);
  await wait(520);
  ...
  audio.tone(190,920,1.2,"sawtooth",.095);
...
function startHeartHold(event) {
  ...
  heartBeatTimer = setInterval(() => {
    ...
    audio.heartbeat(.72 + progress * .55);
    buzz(progress > .7 ? 28 : 18);
  }, 530);
...
$("#openLetterBtn").onclick = async () => {
  ...
  audio.noise(.65,.05,420);
  audio.letterSeal();
  setTimeout(() => audio.sparkle(), 160);
  setTimeout(() => audio.sparkle(), 360);
  buzz([30,40,55]);

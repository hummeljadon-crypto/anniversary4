...
$("#runBtn").onclick = async () => {
  ...
  $("#exitDoor").style.transform = `translateX(-50%) scale(${0.42 + progress * 1.2})`;
  $("#chaseGhost").style.transform = `translateX(-50%) scale(${0.18 + progress * 1.1})`;
  ...
  if (runCount === 10) {
    $("#chaseHint").textContent = "So is she";
...
function startBreath(event) {
  ...
  breathTimer = setInterval(async () => {
    ...
    const progress = Math.min(100, elapsed / 7200 * 100);
    $("#breathFill").style.width = `${progress}%`;

    if (progress > 35) $("#breathHint").textContent = "Don’t move";
    if (progress > 70) $("#breathHint").textContent = "She’s right there";
...
function releaseBreath() {
  ...
  $("#distantMary").style.opacity = "1";
  $("#distantMary").style.transform = "translateX(-50%) translateY(90px) scale(1.2)";
  $("#breathTitle").textContent = "She heard you.";
  audio.tone(220,52,.4,"sawtooth",.1);
  buzz([50,30,90]);

  setTimeout(() => {
    $("#distantMary").removeAttribute("style");
...
const questions = [
  {
    ...
    answers: [
      ...
      { label: "Also me", correct: true }
    ],
    feedback: "Correct. Suspiciously easy."
  },
  {
    text: "Who’s the most beautiful girl in the world?",
    answers: [
      { label: `${CONFIG.girlfriendName} (not even a contest)`, correct: true },
      { label: "Some random person", correct: false }
    ],
    feedback: "Exactly. Not even close."
  },
  {
    text: "Who’s my little baby?",
    answers: [
      { label: CONFIG.girlfriendName, correct: true },
      { label: "Batman", correct: false }
    ],
    feedback: "Correct. Batman is just employed here."
  }
];
...
async function chooseAnswer(answer, question) {
  ...
  if (!answer.correct) {
    $("#answerFeedback").textContent = "Wrong universe. Try again.";

...
class SoundEngine {
  ...
  phoneAlert() {
    this.tone(184, 162, 0.09, "sine", 0.045, -0.18);
    this.tone(184, 162, 0.09, "sine", 0.04, 0.18, 0.13);
  }

  doorCreak() {
    this.noise(0.95, 0.045, 130, "lowpass");
    this.tone(112, 41, 1.05, "triangle", 0.06);
    this.tone(73, 56, 0.72, "sawtooth", 0.018, 0.1, 0.18);
  }

  lightSwitch() {
    this.noise(0.035, 0.075, 1000, "highpass");
    this.tone(1850, 780, 0.055, "square", 0.018);
    this.tone(59, 59, 0.2, "sine", 0.025, 0, 0.06);
  }
  ...
  mirrorWipe() {
    this.noise(0.48, 0.026, 850, "bandpass");
    this.tone(310, 250, 0.42, "triangle", 0.012);
  }
  ...
  shatter() {
    this.noise(0.68, 0.28, 1350, "highpass");
    [760, 1040, 1380, 1820, 2260].forEach((frequency, index) => {
      this.tone(frequency, frequency * 0.2, 0.38 + index * 0.025, "triangle", 0.07, index % 2 ? 0.4 : -0.4, index * 0.018);
    });
    this.tone(68, 31, 0.7, "sine", 0.1);
  }
  ...
  caughtScare() {
    this.stopAmbience(0.06);
    this.noise(0.48, 0.22, 650, "highpass");
    this.tone(245, 35, 0.56, "sawtooth", 0.12);
    this.tone(52, 29, 0.68, "sine", 0.14);
  }
  ...
  capeDrop() {
    this.noise(0.85, 0.052, 420, "bandpass");
    this.tone(115, 43, 0.9, "sawtooth", 0.055);
  }

  landing() {
    this.noise(0.3, 0.16, 130, "lowpass");
    this.tone(86, 29, 0.55, "sine", 0.16);
    this.tone(180, 58, 0.22, "triangle", 0.045, 0, 0.04);
  }

  quizAttack(index) {
    if (index === 0) {
      this.noise(0.12, 0.08, 420, "bandpass", -0.2);
      this.tone(210, 75, 0.28, "triangle", 0.08, 0.25);
    } else if (index === 1) {
      this.tone(160, 620, 0.32, "sawtooth", 0.045, -0.3);
      this.noise(0.16, 0.07, 850, "highpass", 0.3, 0.12);
    } else {
      this.tone(92, 46, 0.42, "sine", 0.12);
      this.noise(0.2, 0.11, 250, "lowpass", 0.1, 0.08);
    }
  }

  wrongAnswer() {
    this.tone(132, 119, 0.2, "triangle", 0.025);
    this.tone(101, 92, 0.18, "triangle", 0.018, 0, 0.12);
  }

  hit() { this.quizAttack(2); }

  evilTransform() {
    this.noise(1.15, 0.055, 450, "bandpass");
    this.tone(128, 34, 1.35, "sawtooth", 0.075);
    this.tone(54, 27, 1.55, "sine", 0.11);
    [620, 760, 910].forEach((frequency, index) => this.tone(frequency, frequency * 0.55, 0.45, "triangle", 0.014, index - 1, 0.35 + index * 0.18));
  }

  slipperWindup() {
    this.noise(0.9, 0.025, 520, "bandpass");
    this.tone(86, 52, 1.0, "sawtooth", 0.045);
  }

  slipperWhoosh() {
    this.noise(1.18, 0.09, 760, "bandpass", -0.6);
    this.tone(170, 1050, 1.16, "sawtooth", 0.07, 0.55);
  }

  slipperImpact() {
    this.stopAmbience(0.04);
    this.noise(0.34, 0.24, 160, "lowpass");
    this.noise(0.2, 0.14, 1300, "highpass", 0, 0.02);
    this.tone(105, 27, 0.54, "sine", 0.18);
    this.tone(410, 92, 0.27, "triangle", 0.08, 0, 0.035);
  }
  ...
  kissSound() {
    this.softNote(329.63, 2.4, 0.014, 0, "sine");
    this.softNote(392, 2.4, 0.012, 0.09, "triangle");
    this.softNote(493.88, 2.6, 0.01, 0.18, "sine");
  }
  ...
  paperOpen() {
    this.noise(0.62, 0.014, 760, "bandpass");
    this.noise(0.28, 0.008, 1300, "highpass", 0.1, 0.18);
    this.softNote(293.66, 1.8, 0.008, 0.25, "sine");
  }
}
...
async function chooseAnswer(answer, question) {
  ...
  $("#battleMary").classList.remove("hit");
